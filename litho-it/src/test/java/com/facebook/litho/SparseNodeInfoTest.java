...
public class SparseNodeInfoTest {
  ...
  public void setup() {
    ...
    mUpdatedNodeInfo = new SparseNodeInfo();
  }
  ...
  private static void testFlagIsSetThenClear(SparseNodeInfo nodeInfo, String flagName) {
    assertThat(isFlagSet(nodeInfo, flagName)).isTrue();
    clearFlag(nodeInfo, flagName);
    assertEmptyFlags(nodeInfo);
  }

  private static boolean isFlagSet(SparseNodeInfo nodeInfo, String flagName) {
    int flagPosition = Whitebox.getInternalState(SparseNodeInfo.class, flagName);
    int flags = Whitebox.getInternalState(nodeInfo, "mPrivateFlags");

    return ((flags & flagPosition) != 0);
  }

  private static void clearFlag(SparseNodeInfo nodeInfo, String flagName) {
    int flagPosition = Whitebox.getInternalState(SparseNodeInfo.class, flagName);
    int flags = Whitebox.getInternalState(nodeInfo, "mPrivateFlags");
    flags &= ~flagPosition;
  ...
  private static void assertEmptyFlags(SparseNodeInfo nodeInfo) {
