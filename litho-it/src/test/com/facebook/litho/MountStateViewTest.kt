...
class MountStateViewTest {
  ...
  fun testViewPaddingAndBackground() {
    ...
    val lithoView = lithoTestRule.render { component }.lithoView
  ...
  fun testComponentDeepUnmount() {
    ...
    val testLithoView = lithoTestRule.render { mountedTestComponent }
    assertThat(lifecycleTracker.isMounted).isTrue
    val viewGroup = ViewGroupWithLithoViewChildren(context.androidContext)
    val child = testLithoView.lithoView
    viewGroup.addView(child)
    testLithoView.setRoot(TestViewComponent.create(context).testView(viewGroup).build())
    assertThat(lifecycleTracker.isMounted).isTrue
  ...
  fun onMountedContentSize_shouldBeEqualToLayoutOutputSize() {
    val component =
        Column.create(context)
            ...
            .build()
    val root = lithoTestRule.render { component }.lithoView
  ...
  fun onMountContentWithPadded9PatchDrawable_shouldNotSetPaddingOnHost() {
    ...
    assertThat(testLithoView.lithoView.paddingTop).isEqualTo(0)
    assertThat(testLithoView.lithoView.paddingRight).isEqualTo(0)
    assertThat(testLithoView.lithoView.paddingBottom).isEqualTo(0)
    assertThat(testLithoView.lithoView.paddingLeft).isEqualTo(0)
