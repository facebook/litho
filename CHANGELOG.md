...
* New: Improved test coverage for the following lifecycle methods: `OnCalculateCachedValue`, `OnCreateTreeProp`, `OnCreateMountContentPool`, `OnCreateLayoutWithSizeSpec`, `OnRegisterRanges/OnEnteredRange`, `OnTrigger`, `OnUpdateState`, `OnUpdateStateWithTransition`.
* New: Improved test coverage for LayoutSpec: layout-diffing, bg updates, applying styles, transition animations, visibility handling; MountSpec: preallocated mount content, mount items after `unmountAll()`, `forceLayout()`.
...
* New: `RecyclerBinder` flag to opt out of caching `StateHandler`s.
...
* Fix: Use common logic for compatibility checks in `setRootAndSizeSpecInternal`.
* Fix: Simplify ComponentTree threading model
