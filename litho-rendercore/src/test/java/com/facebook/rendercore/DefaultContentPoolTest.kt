...
class DefaultContentPoolTest {

  @Test
  fun `acquire - return null if pool is empty`() {
    val pool = MountContentPools.DefaultContentPool(this::class.java, 5)
  ...
  fun `release - should be able to release until pool is full`() {
    val pool = MountContentPools.DefaultContentPool(this::class.java, 1)
    val contentAllocator = FakeContentAllocator()
