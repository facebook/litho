...
class OnLazyStateUpdateMethodSpecBuilder {
  ...
  private final List<TypeMirror> mTypeParameters = new ArrayList<>();
  ...
  public OnLazyStateUpdateMethodSpecBuilder typeParameter(TypeMirror typeParam) {
    mTypeParameters.add(typeParam);
    return this;
  }

  public OnLazyStateUpdateMethodSpecBuilder typeParameters(List<TypeMirror> typeParams) {
  ...
  MethodSpec build() {
    ...
    for (TypeMirror typeParam : mTypeParameters) {
      builder.addTypeVariable(TypeVariableName.get(typeParam.toString()));
    }
