...
class KStateTest {
  ...
  fun useState_calculateLayoutInTwoThreadsConcurrently_stateIsInitializedOnlyOnce() {
    ...
    val view = mLithoTestRule.createTestLithoView()
    ...
    assertThat(getStateHandler(componentTree)?.initialState?.initialStates)
        .describedAs("Initial hook state container is empty")
        .isEmpty()
    assertThat(getStateHandler(componentTree)?.initialState?.pendingStateHandlers)
  ...
  fun useState_synchronousUpdate_stateIsUpdatedSynchronously() {
    ...
    assertThat(stateRef.get()).isEqualTo("hello")
    mLithoTestRule.act(view) { clickOnTag("test_view") }
  ...
  fun useState_reconciliation_stateIsUpdatedWithoutCallingRenderOnSibling() {
    ...
    val view =
        mLithoTestRule.render(widthPx = exactly(100), heightPx = exactly(100)) { RootComponent() }
  ...
  fun useState_updateState_stateIsUpdated_forPrimitive() {
    ...
    val testLithoView = mLithoTestRule.render { TestPrimitiveComponent() }
  ...
  fun useState_reconciliation_stateIsUpdatedWithoutCallingRenderOnSibling_forPrimitive() {
    ...
    mLithoTestRule.act(view) { clickOnTag("Counter: 0") }
