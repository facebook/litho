...
public abstract class ComponentLifecycle implements EventDispatcher, EventTriggerTarget {
   ...
   */
  protected boolean shouldUpdate(Component previous, @Nullable Component next) {
    return !Component.isEquivalentTo(previous, next, /* shouldCompareStateContainers */ true);
  }
  ...
  protected static <E> EventHandler<E> newEventHandler(
    ...
    if (c == null || c.getComponentScope() == null) {
      ...
      ComponentsReporter.emitMessage(
          ...
          WRONG_CONTEXT_FOR_EVENT_HANDLER + ":" + c.getComponentScope().getClass().getSimpleName(),
          String.format(
              ...
              reference.getSimpleName(), c.getComponentScope().getClass().getSimpleName()));
    }
