...
[`DynamicValue<T>`](/javadoc/com/facebook/litho/DynamicValue.html) is the interface that makes it possible.
...
To use this, all you need to do is to create and pass a `DynamicValue<T>` object to the corresponding [`Component.Builder`](/javadoc/com/facebook/litho/Component.Builder.html) method.
...
```
**Second thing** is to create a [`@OnBindDynamicValue`](/javadoc/com/facebook/litho/annotations/OnBindDynamicValue.html) method - *lines 12-17* in `ClockComponentSpec.java` - that should set the value to the mounted content.
