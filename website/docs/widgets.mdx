...
You can find the full list of components and APIs within our [javadocs for the com.facebook.litho.widget package](/javadoc/index.html?com/facebook/litho/widget/package-summary.html).
...
#### Usage
`Text` has numerous optional props you can use to style your text, same as `TextView` since both use `android.text.Layout` under the hood. A full list of them is available in the [javadocs](/javadoc/com/facebook/litho/widget/Text.html).
...
If your card is rendered on top of a dynamically colored background which cannot be "faked" using the `Card` component, check out the less performant [`TransparencyEnabledCard`](/javadoc/com/facebook/litho/widget/TransparencyEnabledCard.html).
