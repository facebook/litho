...
struct LayoutOutputBase {
  ...
  virtual ~LayoutOutputBase() = default;
  ...
  virtual void roundToPixelGrid(const double pointScaleFactor) {
    // This used to depend on NodeType (Default or Text), not sure if we need
    // this now
...
class FLEX_LAYOUT_EXPORT LayoutOutput : public LayoutOutputBase {
 public:
  ...
  void roundToPixelGrid(const double pointScaleFactor) override final {
