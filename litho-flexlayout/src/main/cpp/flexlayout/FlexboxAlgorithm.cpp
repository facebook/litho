...
static auto ResolveAlignment(AlignSelf alignSelf, AlignItems alignItems)
    -> AlignItems {
  switch (alignSelf) {
    case AlignSelf::Auto:
      return alignItems;
...
void facebook::flexlayout::algo::AlgorithmBase::calculateLayout() {
  ...
  for (auto index = size_t{0}; index < numberOfChildren(); ++index) {
    const auto& child = itemStyleAt(index);
    ...
    itemLayoutAt(index).enableTextRounding = child.enableTextRounding;
  }
  ...
  for (auto& flexItem : flexItems) {
    const auto& flexItemStyle = itemStyleAt(flexItem.index);
    if (flexItem.index == singleFlexChildIndex) {
      ...
      if (isDefined(resolvedFlexBasis) && isDefined(availableInnerMainDim)) {
        ...
        if (!isMainAxisRow && isUndefined(resolvedWidth) && isExactWidth &&
            childWidthStretch && isSingleLineContainer) {
        ...
        if (isMainAxisRow && isUndefined(resolvedHeight) && isExactHeight &&
            childHeightStretch && isSingleLineContainer) {
        ...
        const auto resolvedMinWidth =
            flexItemStyle.minWidth.resolve(availableInnerWidth);
        const auto usedMinWidth =
            isDefined(resolvedMinWidth) ? resolvedMinWidth : 0;
        const auto resolvedMaxWidth =
            flexItemStyle.maxWidth.resolve(availableInnerWidth);
        const auto usedMaxWidth = isDefined(resolvedMaxWidth)
            ? resolvedMaxWidth
            : std::numeric_limits<Float>::infinity();

        if (isUndefined(childMinWidth)) {
          childMinWidth = resolvedMinWidth;
        } else {
          childMinWidth =
              std::min(std::max(childMinWidth, usedMinWidth), usedMaxWidth);
        }

        if (isUndefined(childMaxWidth)) {
          childMaxWidth = resolvedMaxWidth;
        } else {
          childMaxWidth =
              std::min(std::max(childMaxWidth, usedMinWidth), usedMaxWidth);
        }
        ...
        ensureItemLayoutAt(
            ...
            availableInnerWidth,
            availableInnerHeight);
        const auto& itemLayout = itemLayoutAt(flexItem.index);
        flexItem.computedFlexBasis =
            isMainAxisRow ? itemLayout.width : itemLayout.height;
      }
    }
  ...
  for (auto i = std::size_t{0}; i < flexItems.size();) {
    ...
    for (const auto& flexItem : flexLine.flexItems) {
      ...
      auto& childLayout = itemLayoutAt(flexItem.index);
      ...
      const auto baseline = margin + cumulativeHeight + [&]() {
        const auto customBaseline =
            customBaselineForItemAt(flexItem.index, width, height);
        if (isDefined(customBaseline)) {
    ...
    if (flexLines.empty()) {
      containerLayout().baseline =
          getLeadingPadding(node, FlexDirection::Column, 0) +
          itemLayoutAt(baselineChild->index).baseline;
    }
      ...
      for (const auto& flexItem : flexLine.flexItems) {
        ...
        if (IsBaselineNode(node, flexItemStyle)) {
          ...
          const auto value = flexLine.maxBaseline -
              itemLayoutAt(flexItem.index).baseline +
          ...
          const auto crossSize = FlexDirectionIsRow(crossAxis)
              ? itemLayoutAt(flexItem.index).width
              : itemLayoutAt(flexItem.index).height;
    ...
    for (const auto& flexItem : flexLine.flexItems) {
      ...
      auto childMainSize = isMainAxisRow ? itemLayoutAt(flexItem.index).width
                                         : itemLayoutAt(flexItem.index).height;
    ...
    const auto usedSize = std::accumulate(
        ...
        [&](Float size, const FlexItem& item) {
          const auto& child = itemLayoutAt(item.index);
    ...
    for (const auto& flexItem : flexLine.flexItems) {
      ...
      const auto directionalOffsetFromMainStartEdgeOfLine = flowIsReverse
          ...
          : offsetFromMainStartEdgeOfLine;
      itemLayoutAt(flexItem.index)
          .setStartPositionOnAxis(
      ...
      const auto crossSize = isMainAxisRow ? itemLayoutAt(flexItem.index).height
                                           : itemLayoutAt(flexItem.index).width;
      ...
      const auto leadingCrossDim = leadingPaddingAndBorderCross + [&]() {
        ...
        switch (alignItem) {
          ...
          case AlignItems::Baseline:
            return isMainAxisRow
                ? flexLine.maxBaseline - itemLayoutAt(flexItem.index).baseline
                : 0.0f;
        }
      }();
    ...
    flexLines.emplace_back(std::move(flexLine));
  }
  ...
  if (node.flexWrap != FlexWrap::NoWrap) {
    ...
    for (const auto& flexLine : flexLines) {
      ...
      for (const auto& flexItem : flexLine.flexItems) {
        ...
        auto crossSize = isMainAxisRow ? itemLayoutAt(flexItem.index).height
                                       : itemLayoutAt(flexItem.index).width;
          ...
          case AlignItems::Stretch: {
            ...
            if (isUndefined(definedCrossSize)) {
              ...
              if (!(FlexLayoutFloatsEqual(
                ...
                crossSize = isMainAxisRow ? itemLayoutAt(flexItem.index).height
                                          : itemLayoutAt(flexItem.index).width;
              }
            }
          } break;
        ...
        if (alignItem != AlignItems::Baseline) {
          if (isMainAxisRow) {
            itemLayoutAt(flexItem.index).top = crossAxisStart;
          } else {
            const auto crossStartEdgeOfContainer =
                crossAxis == FlexDirection::RowReverse
        ...
        if (node.flexWrap == FlexWrap::WrapReverse) {
          if (isMainAxisRow) {
            const auto topPosition = itemLayoutAt(flexItem.index).top;
            itemLayoutAt(flexItem.index).top = newAvailableInnerCrossDim +
                paddingAndBorderAxisCross - topPosition -
                (crossAxisStart + crossSize - topPosition);
          } else {
  ...
  containerLayout().roundToPixelGrid(node.pointScaleFactor);

  // Step 10 Sizing and positioning absolute children
  for (auto i = std::size_t{0}; i < numberOfChildren(); ++i) {
    const auto& child = itemStyleAt(i);
    ...
    const auto w =
        ...
        : child.width;
    const auto h =
        isDefined(child.maxHeight.value) && (child.minHeight == child.maxHeight)
        ? child.maxHeight
        : child.height;
    ...
    const auto absMinHeight =
        isDefined(height) ? height : child.minHeight.resolve(nodeHeight);
    const auto absMaxHeight =
        isDefined(height) ? height : child.maxHeight.resolve(nodeHeight);
    ...
    const auto measuredMainSize =
        isMainAxisRow ? itemLayoutAt(i).width : itemLayoutAt(i).height;
    const auto measuredCrossSize =
        isMainAxisRow ? itemLayoutAt(i).height : itemLayoutAt(i).width;
    ...
    itemLayoutAt(i).setStartPositionOnAxis(
  ...
  for (auto i = std::size_t{0}; i < numberOfChildren(); ++i) {
    ...
    if (itemStyleAt(i).display == Display::None) {
      itemLayoutAt(i).width = 0;
      itemLayoutAt(i).height = 0;
    }
  }
}
