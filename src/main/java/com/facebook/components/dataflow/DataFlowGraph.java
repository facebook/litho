...
public class DataFlowGraph {
  ...
  private void regenerateSortedNodes() {
    ...
    while (!nodesToProcess.isEmpty()) {
      ...
      final ValueNode input = next.getInput();
      if (input == null) {
        continue;
      }
  ...
  private void initializeUninitializedNodes() {
    // Initialize in reverse sorted order
    for (int i = mSortedNodes.size() - 1; i >= 0; i--) {
      final ValueNode node = mSortedNodes.get(i);
  ...
  private void initialize(ValueNode node) {
    // todo: correct initialization
    node.doInitialize(InputSpec.create(false, 0));
  }
}
