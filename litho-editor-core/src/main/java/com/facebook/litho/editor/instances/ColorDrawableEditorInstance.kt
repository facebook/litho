...
class ColorDrawableEditorInstance : Editor {
  ...
  override fun write(f: Field, node: Any?, values: EditorValue): Boolean {
    val colorDrawable = ColorDrawable((values as EditorNumber).value.toInt())
    Reflection.setValueUNSAFE(f, node, colorDrawable)
    return true
  }
}
