...
object ColorComponentSpec {
  ...
  fun onCreateMountContent(c: ComponentContext): ColorDrawable = ColorDrawable()
...
fun onMeasure(
    ...
    size.width = if (SizeSpec.getMode(widthSpec) == SizeSpec.UNSPECIFIED) {
        40
    } else {
    ...
    size.height = if (SizeSpec.getMode(heightSpec) == SizeSpec.UNSPECIFIED) {
        (widthSpec * 1.5).toInt()
    } else {
...
A `@MountSpec` can be defined as pure render by using the `isPureRender` parameter of the `@MountSpec` annotation.
...
fun shouldUpdate(@Prop someStringProp: Diff<String>): Boolean = 
    someStringProp.previous != someStringProp.next
```
