...
Accessibility properties and events for mountable components can be implemented by adding them to the `Style` object that will be returned by the `MountableWithStyle` object from the `render()` method.
...
:::note
In the above example, the returned `MountableWithStyle` object includes `a11y`. This is a reference to the [A11Y Project](https://www.a11yproject.com/), which A11Y describes as a "*a community-driven effort to make digital accessibility easier.*"
:::
