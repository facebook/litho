...
Note that KComponents are only supported in Kotlin - if you need to declare a component from Java, check out the [legacy Spec API](/docs/mainconcepts/uicomposition/layout-specs).
...
- **[State](../../kotlin/use-state/)**: Allows a component to persist private state across updates to the tree.
