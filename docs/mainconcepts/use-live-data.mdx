...
A core principle for `LiveData` observation is the `LifecycleOwner`, which defines at which points the Observer can listen to value changes. In order to use `useLiveData`, you **must** create your `LithoView` with the `AOSPLithoLifecycleProvider` and  pass the appropriate `LifecycleOwner` to it. This can be done as in the sample below:
:::
