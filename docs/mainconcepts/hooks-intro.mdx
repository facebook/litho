...
- **[useEffect](/kotlin/use-effect.mdx)**: lets a component perform side effects when it's attached or detached from the tree.
- **[useRef](/kotlin/use-ref.mdx)**: keeps track of a mutable reference across renders.

### Example: `useState`

To demonstrate usage of hooks in practice, the component below implements a simple counter - whenever it's clicked it'll increment the number of clicks and re-render:
...
As such, there are **three hooks rules** that you need to remember:
 ...
 2. Hook cannot be called conditionally: that means they cannot be called from within conditionals (if/for/while/...), or called **after** an "early exit" check (such as `if (...) return null`).
