...
The most fundamental change in the Litho Kotlin API is that code generation is no longer used. This means there's no need need to mark components as `@LayoutSpecs` or declare any other annotations in the class. Remember to leave off the 'Spec' at the end of the component name!
