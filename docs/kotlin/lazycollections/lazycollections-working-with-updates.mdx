...
Each child in a lazy collection has an id. The id is either assigned by the Developer, or, if no id is provided, then it is generated using the component's type and position.
