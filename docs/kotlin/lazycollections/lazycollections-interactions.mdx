...
* `onNearEnd` will be invoked when the lazy colleciton is scrolled to the last position, or is scrolled within `offset` items away.
* `onNearViewport` will be invoked when the child enters the viewport, or is `offset` items away.

:::note
For more complex scroll handling (e.g. for animations) you can access the `RecyclerView` directly via a `LazyCollectionController` and use a `RecyclerView.OnScrollListener` to recieve callbacks during scroll:
1. Create a `LazyCollectionController` and attach it to the lazy collection via the `lazyCollectionController` parameter. Note that it should be defined in a `useState` hook.
