...
Another Litho plugin that is very useful for debugging Litho is 'Sections'. It can uncover the flow of state changes for the list backed by [Litho Sections](../sections/start.mdx), visualise these changes such as which items were added, reused or deleted, and show the data corresponding to the specific Section.  The following screenshot shows the Sections plugin in action.
