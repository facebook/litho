...
The default value of flexShrink is 1.0f. So, if there is not enough space available for the text to render completely then text will shrink and and will be shown truncated on the screen. Changing the flexShrink value to zero makes sure that text component is not shrinked.
...
However, if the second text is still truncated, based on the requirements, then either make it multiline text or display the two texts inside a Column.
...
In this case, either remove the exact width / height defined so that both siblings can grow and take up the available space or set flexShrink to zero on sibling component.
...
If it is specific to Text, then consider using verticalGravity / horizontalGravity on the Text component to keep it center-aligned:
...
1. Check if the text is cut off due to [flex shrink issues](#why-is-my-text-truncated) or check the setting for [exact width / height on parent components](#if-flexgrowflexshrink-has-been-defined-but-doesnt-do-anything) in the hierarchy.
