...
The arguments to `useEffect` are its dependencies, which control when the effect is cleaned up and invoked again. With `Unit` passed as a dependency arg, the effect lambda is invoked **only once** when a component is mounted for the first time, and the cleanup lambda is invoked **only once** when a component is unmounted for the last time.
