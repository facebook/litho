...
You can find the full list of components and APIs within our [javadocs for the com.facebook.litho.widget package](pathname:///javadoc/index.html?com/facebook/litho/widget/package-summary.html).
...
#### Usage
`Text` has numerous optional props you can use to style your text, same as `TextView` since both use `android.text.Layout` under the hood. A full list of them is available in the [javadocs](pathname:///javadoc/com/facebook/litho/widget/Text.html).
...
`Recycler` is the equivalent of Android's `RecyclerView`. We suggest you to use [Sections](/docs/sections/start) for efficient lists rendering, which is using `Recycler` under the hood.
However, if you really want to use `Recycler` directly, we have an [advanced guide dedicated to it](/docs/sections/recycler-collection-component).
