...
The following code and animation show a simple Component that renders a yellow square then aligns it to either the right or left edge of screen based on value of `toRight`:
...
[`Transition.allLayout()`](https://staticdocs.thefacebook.com/litho/javadoc/com/facebook/litho/Transition.html#allLayout--) creates a transition that automatically animates any changes to position, width, or height, as shown in the following animation.
