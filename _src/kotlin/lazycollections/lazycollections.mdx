...
:::note
Lazy Collections are a Kotlin-only API. If using Java or looking for docs on the older Java codegen-based Sections API, see [Sections Basics](../../sections/start.mdx) in the 'Codegen APIs' section.
:::
...
* **Layouts** - for information on how to customize the rendering of a list, see the [Layout and Styling](lazycollections-layout.mdx) page.
* **Interactions** - to learn how to interact with and listen to events on a list, see the [Interactions](lazycollections-interactions.mdx) page.
* **Performance** - for details on how to make sure a list handles updates efficiently, refer to the [Working with Updates](lazycollections-working-with-updates.mdx) page.
* **Migrating from Sections** - if you're migrating an existing list from the Sections API, have a look at the [Sections Migration Guide](lazycollections-sections-migration.mdx).
