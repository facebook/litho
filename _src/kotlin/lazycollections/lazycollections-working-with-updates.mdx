...
In the following example, there are two `Text` components, one is added conditionally based on `shouldShowGreeting`. If generated ids are used, the first child is always assigned the id "Text:0", so the id would be inconsistent across renders. On Android, you can fix this by manually assigning an `id`.
