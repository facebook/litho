...
During a list update, if a child with the same `id` is found in the old and new lists then the lazy collection will automatically detect if the content has changed. If an update has not occurred we can re-use the subtree as-is, otherwise we'll re-render that child.

Content changes are detected by checking the equality of the component's props. **If any component prop does not implement an `equals()` then it cannot be reused**. It is common for classes to not implement `equals()` e.g. Drawables, Lambdas and data models. A little bit more work is required to avoid unnecessary layouts when these are used.
:::info
**The behaviour will be functionally correct by default.** If you use props that do not provide an `equals()` the UI will still use the most up-to-date prop and state values. However there will be unecessary layouts which will impact performance.
:::

### Using Classes Without `equals()` as Props

If a component accepts a prop that does not implement an `equals()` then it will never be reused. Consider the following example:
...
1. Add an `equals()` to the `Name` class e.g. by making it a `data` class. This approach will not be possible if you're using a an object provided by a framework you don't control.
...
By manually specifying the dependencies `NameComponent` will only be laid out once, and re-used whenever the Example is updated.
...
In this example, the Text component will be unnecessarily laid out on every update to Example because ColorDrawable does not implement an `equals()`.
