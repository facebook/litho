...
Each child in a lazy collection has an id. The id is either assigned by the developer, or if no id is provided an id is generated using the component's type an position.

The id is used to determine how a collection has changed after an update. Children may have been added, removed, changed position, or had their content updated. As an id is unique to a child it is used to match children across changes, so we can create a minimal changeset and apply content update animations etc.

If no `id` parameter is provided then an id is generated based on the child's position and type. This is only sufficient for static content.
...
In the following example there are two `Text` components, and one is added conditionally based on `shouldShowGreeting`. If we used generated ids the first child would always be assigned id "Text:0", so the id would be inconsistent across renders.Android We can fix this by manually assigning an id.
...
Content backed by lists should be added using the `children(..)` function. Specify an `id` lambda that generates an id for each item. Note that it is unacceptable to generate ids using a simple incrementing variable as items may change positions. Instead, use an id from the data model.
