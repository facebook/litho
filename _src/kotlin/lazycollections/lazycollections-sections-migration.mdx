...
1. Replacing the component that renders the section tree with a Lazy Collection component (see [Replace the RecyclerCollectionComponent](#replace-the-recyclercollectioncomponent), below).
2. Adding each component from the section tree as a child of the Lazy Collection (see [Migrating the GroupSectionSpec](#migrating-the-groupsectionspec) and [Migrating the DataDiffSection](#migrating-the-datadiffsection), below).
3. Migrating any event handlers or state (see [Migrating Event Handlers](#migrating-event-handlers) and [Migrating Section State](#migrating-section-state), below).
