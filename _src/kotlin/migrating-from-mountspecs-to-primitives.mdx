...
class FooComponent(
  ...
  override fun PrimitiveComponentScope.render(): LithoPrimitive {
    return LithoPrimitive(
      mountBehavior = MountBehavior(...) {
        bindDynamic(alpha) { content, valueValue ->
          content.alpha = valueValue
          onUnbindDynamic {
            content.alpha = 1f
          }
        }
...
Configuring `hasChildLithoViews` in MountSpec API is done by passing the boolean value to `@MountSpec` annotation. In Primitive Components API there is a `doesMountRenderTreeHosts` flag available inside of `MountConfigurationScope`.
