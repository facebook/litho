...
While `@Prop`s are received from the immediate parent, [`@TreeProp`](../mainconcepts/passing-data-to-components/treeprops.mdx)s can be passed down to a component from any of its ancestors in the current component hierarchy. When testing hierarchies containing components that contain `@TreeProp`, the TreeProp should be passed down to the components as expected. Following is an example LayoutSpec:
