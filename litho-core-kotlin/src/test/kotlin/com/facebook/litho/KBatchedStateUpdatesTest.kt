...
class KBatchedStateUpdatesTest {
  ...
  @Test
  fun stateUpdates_numberOfRendersTriggeredCorrectly_forMountable() {
    val renderCount = AtomicInteger(0)
    ...
    class TestComponent : MountableComponent() {
      override fun MountableComponentScope.render(): MountableRenderResult {
        ...
        return MountableRenderResult(
            TestTextMountable(text = "Counter: ${counter.value}"),
            style =
                Style.height(100.dp).width(100.dp).viewTag("test_view").onClick {
                  // have to block the main looper while we are queueing updates to make
                  // sure that if any scheduling is posted to the main thread, it only
   ...
   */
  @Test
  fun stateUpdates_concurrent_sync_update_consumes_pending_state_updates_and_batching_can_be_resumed_forMountable() {
    val renderCount = AtomicInteger(0)
   ...
   */
  @Test
  fun stateUpdates_concurrent_sync_update_consumes_all_updates_and_consumes_all_batched_updates_forMountable() {
    val renderCount = AtomicInteger(0)
...
internal class TestTextMountable(private val text: String, private val tag: String? = null) :
  ...
  override fun mount(c: Context, content: TextView, layoutData: Any?) {
    ...
    content.tag = tag
    content.contentDescription = tag
  }
